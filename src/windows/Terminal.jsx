...
const Terminal = () => {
    ...
    const [files, setFiles] = useState({ "techStack.txt": true, "resume.pdf": true }); // techStack.txt and resume.pdf already exist
            ...
            <div className="terminal-container" ref={terminalRef}>
                <div className="terminal-output">
                    <div className="terminal-welcome">
                        <p>
                            <Check size={20} className="check" /> Welcome to Jeevant's Terminal
                        </p>
                        <p className="text-gray-400">
                            <Flag size={15} fill="#9ca3af" /> Type 'help' to see available commands
                        </p>
                        <p className="text-gray-500 text-xs mt-2">
                            💡 Try: <span className="text-green-400">ls</span> to see files, then <span className="text-green-400">cat techStack.txt</span> to view your tech stack
                        </p>
                    </div>
                    ...
                    {commandHistory.map((entry, index) => (
                        <div key={index} className="terminal-entry">
                            <div className="terminal-prompt">
                                <span className="font-bold">@Jeevant'sPortfolioTerminal%</span> {entry.command}
                            </div>
                            {entry.output && (
                    ...
                    <form onSubmit={handleSubmit} className="terminal-input-form" onClick={(e) => e.stopPropagation()}>
                        ...
                        <div className="terminal-input-wrapper">
                            <span className="terminal-input-display">
                                {currentCommand}
                                {isFocused && <span className="terminal-cursor">_</span>}
                            </span>
                            <input
                                ref={inputRef}
                                type="text"
